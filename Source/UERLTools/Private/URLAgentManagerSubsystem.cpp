...
void URLAgentManagerSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    UE_LOG(LOG_UERLTOOLS, Log, TEXT("URLAgentManagerSubsystem Initialized."));
    // TODO: Initialize rl_tools global device context if used
    // Example: global_device_context = rlt::malloc(DEVICE, sizeof(typename DEVICE::CONTEXT));
    //          rlt::init(DEVICE, global_device_context);
}
...
void URLAgentManagerSubsystem::Deinitialize()
{
    ...
    {
        RemoveAgent(AgentName); // Ensure proper cleanup for each agent
    }
    ManagedAgents.Empty();

    // TODO: Deallocate rl_tools global device context if used
    // Example: if(global_device_context) rlt::free(DEVICE, global_device_context);
    Super::Deinitialize();
}
...
bool URLAgentManagerSubsystem::ConfigureAgent(FName AgentName, URLEnvironmentComponent* EnvironmentComponent, const FRLTrainingConfig& TrainingConfig)
{
    ...
    // TODO: Use TrainingConfig to initialize rl_tools policy, optimizer, buffers, etc.
    // This will involve allocating memory using rl_tools allocators.
...
bool URLAgentManagerSubsystem::RemoveAgent(FName AgentName)
{
    ...
    if (ManagedAgents.Contains(AgentName))
    {
        ...
        // TODO: CRITICAL - Free all rl_tools resources associated with this agent
        // This includes policy, optimizer, replay buffer, any device contexts specific to this agent.
        // Example: rlt::free(DEVICE, AgentContext.policy_memory);
        UE_LOG(LOG_UERLTOOLS, Log, TEXT("Agent '%s' rl_tools resources should be freed here."), *AgentName.ToString());
